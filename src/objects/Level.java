...
public class Level {
	...
	private int[][] drawMap; // The image drawing memoization array.
	...
	public void populateMap(String fileName, int tileSize) {
		try {
			...
			this.drawMap = new int[this.mapHeight][this.mapWidth];
	...
	public void drawTileImage(int tileValue, int row, int col, Graphics2D g2) {
		if (tileValue != this.drawMap[row][col]) {
			...
			this.drawMap[row][col] = tileValue;
		}
	}
